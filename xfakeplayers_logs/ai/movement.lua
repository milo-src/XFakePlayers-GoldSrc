...
if not Behavior.MoveWhenShooting and IsAttacking() then return

end


if not Behavior.MoveWhenReloading and IsReloading() and IsAttacking() then return

end


if IsPlantingBomb or IsDefusingBomb then return
...
if there are players nearby within a certain distance
...
-- Basic movement when no spec

ific objective is set
...
if GetMaxSpeed() <= 1 then return

end

-- Check

if the navigation area has changed
...
if the area requires crouching
...
if Scenario == ScenarioType.PlantingBomb then
ObjectivePlantingBomb()


if Scenario == ScenarioType.DefusingBomb then
ObjectiveDefusingBomb()


if Scenario == ScenarioType.SearchingBomb then
...
if DeltaTicks(LastStuckMonitorTime) < STUCK_CHECK_PERIOD then return
...
if not HasChain() or ChainIndex > #Chain then return false
...
if not IsWeaponExists(CS_WEAPON_C4) then return
...
if not IsBombPlanted then return
...
if bombPos ~= ChainFinalPoint and IsSlowThink then
ResetObjectiveMovement()
...
if not IsBombDropped then return
