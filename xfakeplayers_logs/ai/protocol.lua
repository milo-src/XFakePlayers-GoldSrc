...
ended version of GetMap() - map name truncated here
...
get field from weapondata_t array for spec
...
ended version to resolve player origin
it using allocated entity for this player to resolve origin
also,

if entity isn't active - using GetRadar() instead
...
enderMode(index: int): int

GetEntityR

enderAmt(index: int): int

GetEntityR

enderFX(index: int): int
...
return weapon index from weaponlist array (WeaponList gmsg event) for using with ext

ended weapon functions
return -1

if weapon isn't exist
...
nav area name contains name of location, where nav area stays (

if location name set) and unique nav area index
...
GetNavAreaPortal(area1, area: int; start,

end: float[3])
ext

ended version of GetNavAreaPortal(...)
...
if entity does not have model or model was not found
...
ife = {Weight = CS_WEAPON_KNIFE_WEIGHT}
...
NAV_AREA_PRECISE = 1 << 2 -- do not adjust for obstacles, just move along area. NAV_AREA_DANGER is synonim ? íå èñïîëüçîâàòü AI_ObstacleAvoidance ?
...
NAV_AREA_CLIFF = 1 << 15 -- this nav area is adjacent to a drop of at least Cl

iffHeight

NAV_AREA_CUSTOM_START = 1 << 16 -- apps may define custom app-spec

ific bits starting with this value
-- custom area flags must be between this two
NAV_AREA_CUSTOM_END = 1 << 26 -- apps must not define custom app-spec

ific bits higher than with this value

NAV_AREA_FUNC_COST = 1 << 29 -- area has designer spec

ified cost controlled by func_nav_cost entities
...
SF_BREAKABLE_ONLY_TRIGGER = 1 << 0 -- If set, this func_breakable will only break when triggered, even

if its health value is set to '1'.
...
SF_BREAKABLE_INSTANT_BREAK = 1 << 8 -- 256 If set, this func_breakable will break instantly

if hit with a crowbar.
