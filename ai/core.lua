...
function Initialization()
    ...
    -- Execute a command if the game directory is "dmc"
    if GetGameDir() == "dmc" then
        ExecuteCommand("_firstspawn") 
    end
    
    -- Handle model and color settings for specific game directories
    ...
    -- Print idle mode status if idle
    if Idle then
        print("Idle mode")
    end
end
...
function Frame() 
    -- Skip processing if in intermission or game is paused
...
function OnTrigger(ATrigger)
    if ATrigger == "RoundStart" then
        -- Reset round-specific states at the start of a round
        IsEndOfRound = false
        Spawn()
