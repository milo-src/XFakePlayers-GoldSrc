-- Check
if there's a chain of commands
function HasChain()
...
if AScenario == ScenarioType.PlantingBomb then
return "Planting Bomb"

if AScenario == ScenarioType.DefusingBomb then
return "Defusing Bomb"

if AScenario == ScenarioType.SearchingBomb then
...
-- Функция для обновления сценария на основе состояния игры
-- Function to update the scenario based on game state

-- Функция для обновления сценария в зависимости от текущей игровой ситуации
-- Function to update the scenario based on the current game state
function UpdateScenario()

-- Изначально задаём сценарий как 'Прогулка'
-- Initially set the scenario to 'Walking'

-- Изначально задаём сценарий как 'Прогулка', что означает обычное перемещение
-- Initially set the scenario to 'Walking', which means normal movement
Scenario = ScenarioType.Walking


-- Если раунд завершён, дальнейшие действия не требуются
-- If the round is over, no further actions are needed
...
-- Проверяем, является ли текущая игра Counter-Strike или Condition Zero
-- Check if the current game is Counter-Strike or Condition Zero
...
-- Функция для нахождения текущего оружия игрока
-- Function to find the player's current weapon
function FindCurrentWeapon()
-- Получаем текущее оружие по его абсолютному индексу
-- Get the current weapon by its absolute index
...
-- Check
if a weapon has secondary ammo
...
if gameDir == "valve" then
return HLWeapons[GetWeaponIndex(AWeapon)].MaxClip
else
print("GetWeaponMaxClip(AWeapon) does not support this game mod
ification")
return nil
...
print("GetWeaponWeight(AWeapon) does not support this game mod
ification")
return nil
...
-- Check
if a weapon needs to be reloaded
...
if GetDistance(I) < Fri
endDistance then
Fri
endDistance = GetDistance(I)
NearestFri
end = I
...
if GetPlayerKills(I - 1) > Fri
endKills then
Fri
endKills = GetPlayerKills(I - 1)
NearestLeaderFri
end = I
